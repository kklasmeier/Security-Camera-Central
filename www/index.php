...
// Get selected camera for filtering
...
// Get total event count (filtered by camera)
...
// Get events for current page (filtered by camera)
$events = $db->get_recent_events($per_page, $offset, $camera_id);
...
<div class="container">
        ...
        <!-- Camera Selector & Per-Page Selector (combined row) -->
        <div class="per-page-selector" style="display: flex; justify-content: space-between; align-items: center;">
            <!-- Camera Selector (left) -->
            <div>
                <?php render_camera_selector('index.php'); ?>
            </div>
            ...
            <div class="events-grid">
                <?php foreach ($events as $event): ?>
                    <?php
                        ...
                        $is_processing = is_video_processing($event['video_h264_path'] ?? '');
                        
                        // Format duration (default 60s if not set)
            ...
            <div class="empty-state">
                ...
                <p class="empty-state-message">
                    <?php if (is_all_cameras_selected()): ?>
                        No motion detection events have been recorded yet.
                        When motion is detected, events will appear here.
                    <?php else: ?>
                        No events found for <?php echo htmlspecialchars(get_camera_display_name($camera_id, $db)); ?>.
                        Try selecting "All Cameras" or a different camera.
                    <?php endif; ?>
                </p>
            </div>
        <?php endif; ?>
